-- FIX (applied live 2026-07-26): the live rollup's orders/revenue counted ONLY
-- in-flight rounds (WHERE r.status='sent') while sent / vouchers / redeemed /
-- spend counted BOTH in-flight and measured rounds. Scope mismatch produced
-- impossible cards: winback showed 111 redemptions but 9 orders (a voucher can
-- only be redeemed ON an order), and "Return (gross)" divided in-flight-only
-- revenue by all-time SMS spend -> 0.5x on a loop actually returning ~8.4x.
-- Orders/revenue now span the same rounds as every other number on the card.
