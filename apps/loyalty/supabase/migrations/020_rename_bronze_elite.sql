...
-- existing reference (UI styles, promotion targeting, analytics queries,
-- hardcoded strings, expired migration history). Only the user-facing
-- `name` and `color` columns change.

UPDATE tiers
   SET name  = 'Member',
       color = '#FFFFFF'
 WHERE id = 'tier-celsius-bronze';

UPDATE tiers
   SET color = '#C0C0C0'
 WHERE id = 'tier-celsius-silver';

UPDATE tiers
   SET color = '#D4AF37'
 WHERE id = 'tier-celsius-gold';

UPDATE tiers
   SET name  = 'Platinum',
       color = '#000000'
 WHERE id = 'tier-celsius-elite';
...
-- / "Everything in Elite" to the new names. Idempotent.
UPDATE tiers
   SET benefits = (
