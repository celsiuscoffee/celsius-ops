...
class SMSNiagaProvider implements SMSProvider {
  ...
  private formatPhone(phone: string): string {
    ...
    return cleaned; // already international or unknown format — pass through
  }
...
class SMS123Provider implements SMSProvider {
  ...
  /**
   * Format phone to Malaysian international format (60XXXXXXXXX).
   * SMS123 expects country code without +.
   */
  private formatPhone(phone: string): string {
    let cleaned = phone.replace(/[\s\-()]/g, '');
