...
import { MapPin, ChevronRight, Coffee, Sparkles, Gift, Clock4, ShoppingCart, QrCode } from "lucide-react-native";
...
export default function Home() {
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      {/* Dine-in entry — scan the table QR to order from your seat. This is
          the in-app path that doesn't depend on the OS deep-link (iOS
          Universal Links are blocked by an Apple provisioning bug; Android
          App Links need the Play signing SHA wired). Lives in the frozen
          header so dine-in is one tap from anywhere on home. */}
      <Pressable
        onPress={() => {
          Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
          // Cast: typed-routes union is generated by Metro and is stale until
          // the next dev/prebuild run; /scan is a real route (app/scan.tsx).
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text
            ...
            numberOfLines={1}
          >
            Scan the QR on your table to order from your seat
          </Text>
        </View>
