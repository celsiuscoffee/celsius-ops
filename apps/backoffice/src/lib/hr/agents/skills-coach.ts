...
export type CoachResult = {
  ...
  cached: boolean;
  audit_count: number;
  // Reason we didn't generate, when insights is null
...
// Compact prompt — we don't ship the photos themselves, just the structured
// rating/notes/date data. The model is good at spotting trend signal from
// numeric series + item names. Keeping the prompt tight (no marketing fluff)
// since the response is JSON-only.
...
export async function getSkillsCoachInsights(userId: string): Promise<CoachResult> {
  // 1. Fetch staff name + completed STAFF audits
  ...
  if (audits.length < 2) {
    // Not enough signal — basic delta view is more honest than a hallucinated
    // trend story from one data point.
    return {
      insights: {
  ...
  // 2. Cache hit?
  const { data: cached } = await hrSupabaseAdmin
  ...
  try {
    ...
    // Coerce missing array fields so the UI doesn't break on a malformed
    // response. summary/needs_more_data are scalars and required.
    insights = {
  ...
  // 4. Cache (upsert by user_id PK)
  await hrSupabaseAdmin.from("staff_skill_coach_cache").upsert(
    {
      user_id: userId,
