...
- menu_margins (VIEW, v2 since 2026-09-02, migration 109) = mirror of the catalog BOM page engine: cheapest active non-ADHOC SupplierProduct catalog price ÷ conversionFactor (NOT product_costs), MenuIngredient lines + per-item PackagingRule lines (cups/lids/boxes included), Hot/Iced × dine-in/takeaway matrix. recipe_cost = blended expected cost (64/36 iced/hot, 50/50 channel); dine_in_cogs / takeaway_cogs = worst-temperature per channel (what the BOM page displays); raw matrix cells exposed too. Optional add-ons (Extra Shot, Oatmilk swap) excluded. ALWAYS quote uncosted_ingredients — a margin with uncosted>0 is overstated.
