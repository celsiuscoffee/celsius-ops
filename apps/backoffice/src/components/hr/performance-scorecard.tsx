...
import { Star, Clock, ClipboardCheck, MessageSquare, AlertTriangle, Trophy, Loader2, ShieldCheck, ThumbsUp, ThumbsDown } from "lucide-react";
...
/**
 * The composite monthly scorecard — attendance, hours, ops compliance, customer
 * reviews and audit mentions per staff member.
 *
 * Was its own page at /hr/performance until the owner asked for the two
 * performance screens to be one (2026-08-03). It is now the "Scorecard" tab of
 * /hr/performance; the period selector lives in the parent so both tabs move
 * together, and the outlet filter stays here because only this tab uses it.
 */
export function PerformanceScorecard({ year, month }: { year: number; month: number }) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-3 rounded-xl border bg-card px-4 py-3.5">
        <label className="flex items-center gap-2 text-sm">
          ...
          <select value={outletId} onChange={(e) => setOutletId(e.target.value)} className="rounded-lg border bg-background px-3 py-2 text-sm">
