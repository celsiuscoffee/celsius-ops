...
import { sendPulse } from "@/lib/agents/pulse";

export const dynamic = "force-dynamic";
export const maxDuration = 60;

// Daily digest for the pulse channel: a plain-English roundup of what the
// agents told each other, learned, and changed in the last 24h. Complements
// the real-time pushes with a single once-a-day summary, then stamps
...
export async function GET(req: NextRequest) {
  ...
  const rows = data ?? [];
  if (rows.length === 0) {
    return NextResponse.json({ sent: false, reason: "no agent messages in the last 24h" });
  }

  const dateStr = new Date().toLocaleDateString("en-MY", { timeZone: "Asia/Kuala_Lumpur", day: "numeric", month: "short" });
  const lines: string[] = [`🤖 <b>Agent activity, ${dateStr}</b>`, `<i>${rows.length} message${rows.length === 1 ? "" : "s"} in the last 24 hours</i>`];
  ...
  return NextResponse.json({ sent, messages: rows.length });
}
