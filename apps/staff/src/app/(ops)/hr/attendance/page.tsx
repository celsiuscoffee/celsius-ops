...
export default function MyAttendancePage() {
  ...
  const { data, error, mutate } = useFetch<{ logs: AttendanceLog[]; stats: Stats }>(`/api/hr/attendance?days=${days}`);
    ...
    <div className="px-4 pt-6">
      ...
      <div className="mb-6 grid grid-cols-3 gap-2">
        ...
        <div className="rounded-xl border border-gray-100 bg-white p-3 text-center">
          ...
          {stats.pendingOT > 0 && (
            <p className="text-[9px] text-amber-600" title="Paid once your manager approves the attendance">+{stats.pendingOT}h awaiting approval</p>
          )}
        </div>
      </div>
      ...
      {/* Daily logs */}
      {!data && error ? (
        <FetchError error={error} onRetry={() => mutate()} what="your attendance" />
      ) : logs.length === 0 ? (
                  ...
                  {dayLogs.map((log) => {
                    ...
                    return (
                      <div key={log.id} className="flex items-center gap-2 text-xs">
                        ...
                        {/* OT accrues in half-hour brackets (min 0.5h), same
                            floor as payroll and the stats above; the old >= 1
                            hid a 0.5h tail that the API was already counting. */}
                        {Number(log.overtime_hours) >= 0.5 && (
                          log.ai_status === "approved" || log.final_status === "approved" ? (
                            <span className="font-medium text-blue-600">+{log.overtime_hours}h OT</span>
                          ) : (
                            <span className="font-medium text-amber-600" title="Paid only once your manager approves this attendance">
