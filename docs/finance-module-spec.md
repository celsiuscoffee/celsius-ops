...
3. **Bank feed + Matcher** — the cutover gate. ✅ **Matcher rules engine built** (`agents/matcher-rules.ts` pure + `agents/matcher.ts` IO + `/api/finance/match` trigger): reconciles each `fin_bank_transactions` line against open `fin_invoices` (AR) / `fin_bills` (AP) by exact reference, exact amount within a date window, and rounding tolerance; ≥0.90 auto-writes `fin_matches` + advances `payment_status`/`paid_amount`, sub-threshold → `match` exceptions with the best proposal. Sign routes AR vs AP; one-deposit-can't-match-two via in-memory outstanding decrement. ✅ **Bank-feed landing zone built** (`ingestors/bank-feed*.ts`): source-agnostic `ingestBankLines()` normalizes lines from any source into `fin_bank_transactions`, idempotent via the composite unique constraint (absent reference coerced to `""` so PG's distinct-NULLs don't defeat dedupe). ✅ **Bukku adapter built** (`ingestors/bukku-bank*.ts` + `/api/finance/bukku-sync` + orchestrator step 0). Verified contract (reverse-engineered from `developers.bukku.my/specs/bukku-bank-api.yaml` via Supabase `pg_net`, since sandbox egress was blocked): base `https://api.bukku.my`, Bearer token (company-scoped — the token selects the company, no company header), `GET /banking/incomes` (Money In, inflow +) and `GET /banking/expenses` (Money Out, outflow −), query `date_from/date_to/page/page_size`, paginated via `paging.{current_page,per_page,total}`, RM float amounts. Each entry → `BankLineInput` using `deposit_items[].account_code` (Bukku COA = ours, seeded from Bukku) as the bank account, `date`, signed `amount`, `number`/`number2` as reference, `description`. `syncBukkuBankFeed` loops Bukku-enabled outlets, per-outlet failure-isolated, lands all via `ingestBankLines`. **Operational note (go-live):** there is **no raw bank-feed-line endpoint** — Bukku's feed becomes Money In/Out entries, so we mirror those. A valid **API access token** is required per Bukku company (Control Panel → Integrations → API Access). The token in `_outlet_settings_backup.bukku_token` is **stale** (live call returned `403 "User is not authorised to access the company"`), and the live `Outlet.bukkuToken` column is empty + `bukkuEnabled=false`. To go live: enter a current token per outlet in Settings → Integrations (writes `Outlet.bukkuToken`, flips `bukkuEnabled`), then **verify with `GET /api/finance/bukku-sync`** — a read-only probe that reports auth status, income/expense counts, and a mapped sample per enabled outlet (no ingest), so a stale token shows as `403` before any real sync. Once it's green, `POST /api/finance/bukku-sync` (or the nightly cron's step 0) lands the lines. The production backoffice has normal egress so it will reach `api.bukku.my` (the sandbox's egress allowlist only matters for in-session testing). Topology: bank → Bukku feed → pull Money In/Out → us; we keep only date/amount/description/reference/account and discard Bukku's GL coding. (b) the LLM residual pass for fee-net card settlements, batched deposits, and partial payments; ✅ **cash-clearing journal posts on every match** (`agents/clearing.ts`): commitMatch now posts DR bank / CR channel-debtor(+SST) for AR and DR payable / CR bank for AP, so the GL cash position is correct (not just `paid_amount`). The AR split mirrors ar.ts exactly (channel debtor at net + SST in 1000-02), so clearing unwinds what was booked with no AR-agent change. Posts first so a period-lock/balance failure leaves the line retryable. `transaction` targets and partials handled. ✅ **AR candidates now exist** — on a confident post the AR agent emits one channel-tagged `fin_invoices` row per non-zero channel (`agents/ar-invoices.ts`), `total` = channel net + its proportional SST share (what the bank receives), deterministic `invoice_number` for idempotency. The Matcher reconciles bank settlements against these. Low-confidence days emit no invoices until the channel-split exception is resolved.
4. **AP autopilot** — ✅ works from manual upload. Add supplier-doc **email inbox** ingestion (replaces manual courier step).
5. **Exception resolvers (all types)** — ✅ **`match` resolver built.** `inbox.ts` now resolves match exceptions: `approve` accepts the Matcher's proposal, `match` is an explicit human pick, `dismiss` flags the bank line `ignored`. All go through the shared `commitMatch` (same accounting as the auto-Matcher) and tag the Matcher's `fin_agent_decisions` row as the training signal (confirmed vs corrected). `/api/finance/exceptions/:id/resolve` accepts the `match` action. Still pending: AR-`categorization` resolution (re-post the day with a corrected channel split) and anomaly / missing-doc / out-of-balance resolvers.
...
8. **Anomaly + polish** — ✅ **Anomaly sweep built** (`agents/anomaly-detectors.ts` pure + `agents/anomaly.ts` IO + `/api/finance/anomaly` trigger): deterministic, raise-only self-check producing `duplicate` (same supplier + bill number), `out_of_balance` (posted txn debits≠credits), `missing_doc` (posted AR/AP with no source doc), and `anomaly` (bill far above the supplier's own 12-month median — needs both a 4× multiple and a material absolute jump, so cheap suppliers don't spam). Idempotent (one open exception per row+type); the inbox surfaces them (dismiss works today; type-specific approve/reverse resolvers are a follow-up). ✅ **Nightly orchestration wired** — `lib/finance/orchestrator.ts` `runNightlyClose(date)` chains EOD ingest → Matcher (trailing 7-day window, catches late settlements) → Anomaly sweep, each idempotent and failure-isolated; the `finance-eod` cron (4 AM MYT) now runs the whole loop. Still pending: WhatsApp digest, Ask box (NL queries), and the **Fable 5 nightly orchestrator** layered on top — it investigates non-reconciling outlets and writes the digest, and does not categorize individual lines (Haiku/Sonnet stay on per-line work).
